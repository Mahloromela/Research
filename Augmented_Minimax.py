...
from Neural_network import NNAgent, load_most_recent_model
# from Morabaraba.Training5.Preprocessing import count_potential_mills, count_mills, blocked_potential_mills, \
#     configuration_count, mobility, central_control, blockades, piece_stability
...
class AugmentMinimaxAgent5(Player):
    ...
    def best_move(self, game):
        ...
        alpha = float('-inf')
        beta = float('inf')
        board_state = np.array([scaler.fit_transform(self.board_to_numpy(game.Board.board, game).reshape(-1, 1)).flatten()])
