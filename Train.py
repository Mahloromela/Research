...
def main():
    ...
    # Load game states
    for file_name in os.listdir(path=r'/home/vmuser/Pictures/Code/Morabaraba/Data8'):
        if file_name.endswith(".json"):
            game_states = processor.load_game_states(rf'/home/vmuser/Pictures/Code/Morabaraba/Data8/{file_name}')
    ...
    evaluate_value_model(X_test_placement, v_test_placement, agent.placement_value_model, 'Placement', 24)
    # evaluate_individual_predictions(X_test_placement, v_test_placement, agent, 'Placement')

    print("Evaluating movement value model...")
    evaluate_value_model(X_test_movement, v_test_movement, agent.movement_value_model, 'Movement', 24)
    # evaluate_individual_predictions(X_test_movement, v_test_movement, agent, 'Movement')

    print("Evaluating jumping value model...")
    #evaluate_value_model(X_test_jumping, v_test_jumping, agent.jumping_value_model, 'Jumping', 24)
