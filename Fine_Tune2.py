...
import os
from Neural_network2 import save_model_with_unique_name, NNAgent
from Preprocessing2 import MorabarabaDataProcessor


os.environ["CUDA_VISIBLE_DEVICES"] = "-1"

def reshape_for_conv2d(X_data, board_size):
    return np.array(X_data).reshape(-1, 32, 1)
...
def evaluate_individual_predictions(X_test, v_test, model, stage_name):
    ...
    for index, test_data in enumerate(X_test):
        test_data_reshaped = test_data.reshape(1, 32, 1)
...
def main():
    ...
    # Load game states
    for file_name in os.listdir(path=r'/home/vmuser/Pictures/Code/Morabaraba/Data6'):
        if file_name.endswith(".json"):
            game_states = processor.load_game_states(
                rf'/home/vmuser/Pictures/Code/Morabaraba/Data6/{file_name}')
    ...
    evaluate_value_model(X_test_placement, v_test_placement, agent.placement_value_model, 'Placement', 32)
    # evaluate_individual_predictions(X_test_placement, v_test_placement, agent, 'Placement')

    # print("Evaluating movement value model...")
    # evaluate_value_model(X_test_movement, v_test_movement, agent.movement_value_model, 'Movement', 32)
    # # evaluate_individual_predictions(X_test_movement, v_test_movement, agent, 'Movement')

    # print("Evaluating jumping value model...")
    # evaluate_value_model(X_test_jumping, v_test_jumping, agent.jumping_value_model, 'Jumping', 32)
